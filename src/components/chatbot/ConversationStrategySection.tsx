...
const ConversationStrategySection = ({ onSaved }: { onSaved?: () => void }) => {
  ...
  const handleSave = () => {
    setSaving(true);
    api.putConversationStrategy(data)
      .then(() => {
        toast({ title: "Saved ✓", description: "Conversation strategy updated." });
    ...
    <div className="p-6 space-y-5">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Book a 30-minute discovery call, Get them to fill out our form..."
        />
      ...
      <div>
        ...
        <div className="space-y-2">
          {PRICE_OPTIONS.map((p) => (
            <button
              key={p.value}
              onClick={() => update({ price_reveal: p.value })}
              className={`w-full text-left rounded-lg p-3 border transition-all ${
                data.price_reveal === p.value
                  ? "border-primary bg-primary/5 shadow-[0_0_8px_hsl(48_92%_53%/0.1)]"
                  : "border-border bg-card hover:border-muted-foreground/30"
              }`}
            >
              <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full border-2 flex items-center justify-center ${
                  data.price_reveal === p.value ? "border-primary" : "border-muted-foreground"
                }`}>
